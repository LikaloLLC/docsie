...
大規模言語モデル（LLM）はプロンプトのコンテキストを理解する必要があります。システムに少しの背景知識を入力することで、LLMが望ましい出力を提供するよう訓練できます。Docsieでの技術文書作成では、迅速で確実なサポートや質問解決のためにLLMチャットボットを導入できます。例えば、AIシステムに磁石の仕組みを6年生向けに説明するようコンテキストを追加できます。
